                ...
                <{if isset($visibilities.$field_id)}>
                    <ul>
                        <{foreach item=visibility from=$visibilities.$field_id}>
                            ...
                            <li>
                                <{$smarty.const._AM_SONGLIST_FIELDVISIBLEFOR}> <{$groups.$user_gid}>
                                <{$smarty.const._AM_SONGLIST_FIELDVISIBLEON}> <{$groups.$profile_gid}>
                                <a href="visibility.php?op=del&amp;field_id=<{$field_id}>&amp;ug=<{$user_gid}>&amp;pg=<{$profile_gid}>" title="<{$smarty.const._DELETE}>">
                                    <img src="<{$xoops_url}>/modules/songlist/images/icons/16/no.png" alt="<{$smarty.const._DELETE}>">
                                </a>
                            </li>
                        <{/foreach}>
                    </ul>
                <{else}>
                    <{$smarty.const._AM_SONGLIST_FIELDNOTVISIBLE}>
                <{/if}>
            </td>
        </tr>
    <{/foreach}>
