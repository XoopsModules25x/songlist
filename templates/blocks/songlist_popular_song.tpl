<{if $block}>
    <div style='font-size:1.15em; clear:both;'>
        <{if $block.title|default:''}>
            <{$block.title}>
        <{elseif $block.name|default:''}>
            <{$block.name}>
        <{/if}>
    </div>
    <{if $block.categories_array|default:''|is_array && count($block.categories_array) > 0 }>
    ...
    <{elseif $block.category|default:''}>
        <div style='font-size:0.85em; clear:both;'>
            <{$smarty.const._BL_SONGLIST_CATEGORY}>
        </div>
        <div style='font-size:0.65em; clear:both;'>
            <{$block.category.title|default:''}>
        </div>
    <{/if}>
    <{if $block.genre_array|default:''|is_array && count($block.genre_array) > 0 }>
        <div style='font-size:0.85em; clear:both;'>
            <{$smarty.const._BL_SONGLIST_GENRES}>
        </div>
        <div style='font-size:0.65em; clear:both;'>
            <{foreach from=$block.genre_array key=number item=genre}>
                <{$genre.name}><{if ($number<sizeof($block.genre_array)-1)}>,&nbsp;<{/if}>
            <{/foreach}>
        </div>
    ...
    <{/if}>
    <{if $block.artists_array|is_array && count($block.artists_array) > 0 }>
        <div style='font-size:0.85em; clear:both;'>
